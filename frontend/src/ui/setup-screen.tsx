      ...
      <Tabs defaultValue="people" className="gap-5">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (locked.length) {
                          if (
                            !confirm(
                              `${person.name} appears in ${plural(locked.length, "month")} already locked (${locked.map((k) => monthLabel(k, true)).join(", ")}).\n\nThose months stay exactly as they were charged. ${person.name} will be archived — off the roster and out of future months, but still on Settle until settled.\n\nArchive them?`,
                            )
                          )
                            return;
                          store.mutate(() => {
        ...
        >
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Weight scales how much a room counts toward rent — a bathroom at 0.5× is treated as half
            its floor area. Private rooms are paid for by whoever is liable for them; shared rooms
